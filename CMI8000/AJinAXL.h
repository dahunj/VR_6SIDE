...
public:
	...
	void Move_Absolute(int nAxis, double dPos, double dSpeed=0.0);
	void Move_Absolute_Vel(int nAxis, double dPos, double overrideSpeed);
	void Move_AbsSlow(int nAxis, double dPos);	// 추가 2019.11.05
	void Move_Relative(int nAxis, double dPos, double dSpeed=0.0);
	...
	void SetEncoderActPos(int nAxis, double dPos);
	...
	void Save_MotionParamForAll(int nAxis);
