...
BEGIN_MESSAGE_MAP(CSetupDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_SETUP_SAVE_ALLPARAM, &CSetupDlg::OnBnClickedBtnSetupSaveAllparam)
END_MESSAGE_MAP()
...
void CSetupDlg::OnBnClickedBtnSetupSaveAllparam()
{
	m_pSetupEquipDlg->Save_EquipDataForAllParam();
	m_pSetupMoveDlg->Save_MoveDataForAllParam();
	m_pSetupMotionDlg->Save_MotionDataForAllParam();
}
