// SetupMotionTabDlg.cpp : ±∏«ˆ ∆ƒ¿œ¿‘¥œ¥Ÿ.
//
#include "stdafx.h"
#include "CMI8000.h"
...
void CSetupMotionTabDlg::OnShowWindow(BOOL bShow, UINT nStatus) 
{
	...
	int nMaxAxis = (m_nMotionTab == 5 ? 6 : 8);
	...
	for (int i = 0; i < 8; i++) {
		if(nStartAx==16 && i==0){ //16 Axis remove
			
			m_stcAxisName[i].ShowWindow(FALSE);
			m_stcAxisPos[i].ShowWindow(FALSE);
			m_stcAxisVel[i].ShowWindow(FALSE);
			m_ledAxisSOn[i].ShowWindow(FALSE);
			m_ledAxisOrg[i].ShowWindow(FALSE);
			m_ledAxisElp[i].ShowWindow(FALSE);
			m_ledAxisEln[i].ShowWindow(FALSE);
			m_ledAxisAlm[i].ShowWindow(FALSE);
			m_ledAxisInP[i].ShowWindow(FALSE);
			m_ledAxisRun[i].ShowWindow(FALSE);
			m_ledAxisHom[i].ShowWindow(FALSE);

			m_Group[i + 2].ShowWindow(FALSE);
			for (int j = 0; j < 5; j++) m_Label[i * 5 + j + 22].ShowWindow(FALSE);

			m_chkSOn[i].ShowWindow(FALSE);
			m_btnReset[i].ShowWindow(FALSE);
			m_btnHome[i].ShowWindow(FALSE);
			m_btnStop[i].ShowWindow(FALSE);
			m_btnJogP[i].ShowWindow(FALSE);
			m_btnJogN[i].ShowWindow(FALSE);
			m_stcAbsDist[i].ShowWindow(FALSE);
			m_btnAbsMove[i].ShowWindow(FALSE);
			m_stcRelDist[i].ShowWindow(FALSE);
			m_btnRelMoveP[i].ShowWindow(FALSE);
			m_btnRelMoveN[i].ShowWindow(FALSE);
			m_stcSpeedM[i].ShowWindow(FALSE);
			m_stcSpeedJ[i].ShowWindow(FALSE);
			m_stcAccel[i].ShowWindow(FALSE);
			continue;
		}
...
{
	for (int i = 0; i < 10; i++) m_Group[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x00, 0x00, 0xFF), COLOR_DEFAULT);
	for (int i = 0; i < 62; i++) m_Label[i].Init_Ctrl("πŸ≈¡", 10, FALSE, RGB(0xFF, 0xFF, 0xFF), RGB(0x60, 0x60, 0x60));

	for (int i = 0; i < 8; i++) m_stcAxisName[i].Init_Ctrl("πŸ≈¡", 10, FALSE, RGB(0xFF, 0xFF, 0xFF), RGB(0x80, 0x00, 0x00));
	for (int i = 0; i < 8; i++) m_stcAxisPos[i].Init_Ctrl("πŸ≈¡", 11, TRUE, RGB(0xFF, 0x00, 0x00), RGB(0xE0, 0xE0, 0xE0));
	for (int i = 0; i < 8; i++) m_stcAxisVel[i].Init_Ctrl("πŸ≈¡", 11, TRUE, RGB(0x00, 0x00, 0xFF), RGB(0xE0, 0xE0, 0xE0));
	...
	for (int i = 0; i < 8; i++) m_chkSOn[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0xFF, 0x63, 0x47), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_btnReset[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x00, 0x80, 0x00), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_btnHome[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x22, 0x8B, 0x22), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_btnStop[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0xFF, 0x00, 0x00), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_btnJogP[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x80, 0x00, 0x80), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_btnJogN[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x80, 0x00, 0x80), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_stcAbsDist[i].Init_Ctrl("πŸ≈¡", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0xFF, 0xFF, 0xC0));
	for (int i = 0; i < 8; i++) m_btnAbsMove[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x00, 0x00, 0xFF), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_stcRelDist[i].Init_Ctrl("πŸ≈¡", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0x80, 0xFF, 0xC0));
	for (int i = 0; i < 8; i++) m_btnRelMoveP[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x00, 0x00, 0xFF), COLOR_DEFAULT, 0, 0);
	for (int i = 0; i < 8; i++) m_btnRelMoveN[i].Init_Ctrl("πŸ≈¡", 12, TRUE, RGB(0x00, 0x00, 0xFF), COLOR_DEFAULT, 0, 0);

	for (int i = 0; i < 8; i++) m_stcSpeedM[i].Init_Ctrl("πŸ≈¡", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0xFF, 0xEF, 0xD5));
	for (int i = 0; i < 8; i++) m_stcSpeedJ[i].Init_Ctrl("πŸ≈¡", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0xFF, 0xEF, 0xD5));
	for (int i = 0; i < 8; i++) m_stcAccel[i].Init_Ctrl("πŸ≈¡", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0xC0, 0xFF, 0xF0));
}
