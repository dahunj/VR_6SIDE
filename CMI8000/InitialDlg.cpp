...
void CInitialDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (pDX14->iStartSw && !m_rdoInitStart.GetCheck()) {
		g_objLogFile.Save_HandlerLog("[Initial Mode] START S/W Push");
		m_rdoInitStart.SetCheck(TRUE);

	} else if (pDX14->iStopSw && !m_rdoInitStop.GetCheck()) {
...
void CInitialDlg::Initial_Controls()
{
	...
	for (int i = 0; i < 3; i++) m_ledMainAir[i].Init_Ctrl("바탕", 11, FALSE, COLOR_DEFAULT, COLOR_DEFAULT, CLedCS::emGreen, CLedCS::em16);
	for (int i = 0; i < 6; i++) m_ledEmgSw[i].Init_Ctrl("바탕", 11, FALSE, COLOR_DEFAULT, COLOR_DEFAULT, CLedCS::emRed, CLedCS::em16);
	for (int i = 0; i < 19; i++) m_ledDoorOpen[i].Init_Ctrl("바탕", 11, FALSE, COLOR_DEFAULT, COLOR_DEFAULT, CLedCS::emGreen, CLedCS::em16);
...
void CInitialDlg::Display_Status()
{
	...
	m_ledMainAir[2].Set_On(pDX14->iMainAir3);
	...
	for (int i = 0; i < 19; i++) m_ledDoorOpen[i].Set_On((pDX15->nValue >> i) & 1);
	...
	for (int i = 0; i < 14; i++) { strTemp.Format("%03d", *(pCase + i)); m_stcInitCase[i].Set_Text(strTemp); }
}
...
void CInitialDlg::Display_Initial() 
{
	...
	if (*(pCase + 2) < 90 || *(pCase + 7) < 90) {												// Angle
	...
	if (*(pCase + 3) < 90 || *(pCase + 4) < 90 || *(pCase + 8) < 90) {							// Top & Bottom
	...
	if (*(pCase + 5) < 90 || *(pCase + 9) < 90 || *(pCase + 10) < 90 || *(pCase + 11) < 90) {	// Unload
	...
	if (*(pCase + 5) < 12 || *(pCase + 6) < 13) {												// Empty
